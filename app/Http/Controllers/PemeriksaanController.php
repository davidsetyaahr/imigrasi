...
class PemeriksaanController extends Controller
{
    public function pemeriksaan($id_jadwal)
    {
        ...
        $data['pertanyaan'] = \DB::table('jawab_pertanyaan as jp')->select('p.pertanyaan','jp.jawaban','jp.id_jawab')->join('pertanyaan as p','jp.id_pertanyaan','p.id_pertanyaan')->where('id_pengajuan',$data['detail']->id_pengajuan)->get();
    ...
    public function diperiksa(request $request)
    {
        ...
        foreach ($request->input('jawaban') as $key => $value) {
            ...
            ->where('id_jawab',$request->input('id_jawab')[$key])
            ->update(['jawaban' => $value]);
        }

        \DB::table('pengajuan')
        ->where('id_pengajuan',$id_pengajuan)
        ->update(['no_pemeriksaan' => $request->input('nomor'), 'tgl_pemeriksaan' => date('Y-m-d H:i:s'),'id_petugas' => auth()->user()->id_petugas]);
